...
\begin{document}
\section*{Doubly Linked List Before Operation}
\begin{tikzpicture}
    ...
    \path[*->] let \p1 = (N5.three), \p2 = (N5.center) in (\x1,\y2) edge [bend left] ($(N6.one)+(0,0.2)$);
    \path[*->] ($(N6.one)+(0.1,0.1)$) edge [bend left] ($(N5.three)+(0,-0.05)$);
    \path[*->] let \p1 = (N6.three), \p2 = (N6.center) in (\x1,\y2) edge [bend left] ($(N7.one)+(0,0.2)$);
    \path[*->] ($(N7.one)+(0.1,0.1)$) edge [bend left] ($(N6.three)+(0,-0.05)$);
\end{tikzpicture}
\section*{Doubly Linked List After Operation}
\begin{tikzpicture}
    \node[list,on chain] (N1) {\nodepart{second} 12};
    \node[list,on chain] (N2) {\nodepart{second} 99};
    \node[list,on chain] (N3) {\nodepart{second} 100};
