...
def generate_latex_binary_tree(node, target=None, is_root=False):
    ...
    if node.value == target:
        latex_code = f"{node_prefix}node[fill=pink!60] {{{node.value}}}"
    else:
...
def main():
    ...
    filename = "binary_tree_bfs_steps.tex"

    with open(filename, "w") as file:
        file.write(latex_document)

    print(f"LaTeX code for binary tree BFS steps has been generated and saved to {filename}")
